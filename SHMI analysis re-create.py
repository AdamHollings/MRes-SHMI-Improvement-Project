...
# DBTITLE 1,Checking the dates are im range
# print(HES_2021.toPandas().DISDATE.unique())
# print(HES_1718.toPandas().DISDATE.unique())

# COMMAND ----------

# DBTITLE 1,Change to pandas
SHMI_HES_cohort_pdf = SHMI_HES_cohort.select('FYEAR', 'PSEUDO_HESID', 'SUSSPELLID','DIAG_1', 'EPIORDER', 'P_SPELL_DISDATE').toPandas()

# COMMAND ----------

SHMI_HES_cohort_pdf.groupby(['SUSSPELLID']).count()#.query("index == '1621526571308999571'")

# COMMAND ----------

# DBTITLE 1,Display the year counts
display(SHMI_HES_cohort[['SUSSPELLID','FYEAR']].groupby('FYEAR').count())

# COMMAND ----------

# DBTITLE 1,Data Shape of HES_2021
SHMI_HES_cohort_pdf.shape # this pretty much matches whats in the SQL server

# COMMAND ----------

# DBTITLE 1,Check how many episodes per spell
high_episode_numbers = SHMI_HES_cohort_pdf[['PSEUDO_HESID', 'SUSSPELLID','DIAG_1', 'EPIORDER']].groupby('SUSSPELLID').count().query("PSEUDO_HESID > 3")
SHMI_HES_cohort_pdf[SHMI_HES_cohort_pdf.SUSSPELLID == '1529319371007976358'] # a large number of episodes in one spell, 11. Just checking it is true. Panic disorder as the main DIAG

# COMMAND ----------

SHMI_HES_cohort_pdf[SHMI_HES_cohort_pdf.SUSSPELLID == '1589842174236995816']

# COMMAND ----------

SHMI_HES_cohort_pdf.query("PSEUDO_HESID == 'EA34800A9BECF8884B8A7625E7D0522C'").sort_values(['SUSSPELLID', 'EPIORDER'])
